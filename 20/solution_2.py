...
def read_input():
    ...
    for line in fileinput.input():
        # convert to int
        number = int(line)
        all_numbers.append(number * decryption_key)
    return all_numbers
...
def move_node(current_node, num_moves, total_nodes):
    num_moves = num_moves % (total_nodes - 1)
    while num_moves != 0:
        if num_moves < 0:
...
def move_all_nodes(all_nodes):
    for node in all_nodes:
        move_node(node, node.number, len(all_nodes))

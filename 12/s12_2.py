...
def is_move_allowed(current_pos: Tuple[int, int], move: Tuple[int, int, int], height_map: List[List[str]], fastest_path_map: List[List[int]]) -> bool:
    ...
    if move[0] < 0 or move[1] < 0:
        ...
        return False
    elif ord(height_map[current_pos[1]][current_pos[0]]) - ord(height_map[move[1]][move[0]]) > 1:
        return False
...
def find_fastest_path_map(start_pos: Tuple[int, int], height_map: List[List[str]]) -> List[List[int]]:
    """
...
def main():
    ...
    fastest_dist = find_fastest_path_map(end_pos, height_map)
    print(fastest_dist)
